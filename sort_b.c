...
void	calculate_required_moves(t_stacks *stacks)
{
	...
	while (i < stacks->b_len)
	{
		...
		// ft_printf("(rotate a %d, reverse rotate a: %d, rotate b: %d, reverse rotate b: %d)\n", stacks->b[i].required_moves.rotate_a, stacks->b[i].required_moves.reverse_rotate_a, stacks->b[i].required_moves.rotate_b, stacks->b[i].required_moves.reverse_rotate_b);
